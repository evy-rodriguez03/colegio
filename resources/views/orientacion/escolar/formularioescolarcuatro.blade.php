        ...
        <form class="row g-3 mt-3" action="{{route('escolar.updatecuatro', $alumnos->id) }}" method="POST">
            ...
            <table class="table align-items-center table-flush">
                <tr class="thead-light">
                    <td><input type="checkbox" name="abanda" id="abanda" value="1"
                    @if(old('abanda') == '1' || (isset($escolar->abanda) ? $escolar->abanda : '') == '1')
                            checked
                        @endif><label for="abanda"> Banda de Guerra</label></td>
                    <td><input type="checkbox" name="afutbol" id="afutbol" value="1"
                    @if(old('afutbol') == '1' || (isset($escolar->afutbol) ? $escolar->afutbol : '') == '1')
                            checked
                        @endif><label for="afutbol"> Futbol</label></td>
                    <td><input type="checkbox" name="apingpong" id="apingpong" value="1"
                    @if(old('apingpong') == '1' || (isset($escolar->apingpong) ? $escolar->apingpong : '') == '1')
                            checked
                        @endif> <label for="apingpong">Ping Pong</label></td>
                    <td><input type="checkbox" name="anumeros" id="anumeros" value="1"
                    @if(old('anumeros') == '1' || (isset($escolar->anumeros) ? $escolar->anumeros : '') == '1')
                            checked
                        @endif> <label for="anumeros"> Trabajo con Numeros</label></td>
                    <td><input type="checkbox" name="alectura" id="alectura" value="1"
                    @if(old('alectura') == '1' || (isset($escolar->alectura) ? $escolar->alectura : '') == '1')
                            checked
                        @endif> <label for="alectura"> Lectura</label></td>
                </tr>
                <tr>
                    <td><input type="checkbox" name="acoro" id="acoro" value="1"
                    @if(old('acoro') == '1' || (isset($escolar->acoro) ? $escolar->acoro : '') == '1')
                            checked
                        @endif><label for="acoro"> Coro</label></td>
                    <td><input type="checkbox" name="abasket" id="abasket" value="1"
                    @if(old('abasket') == '1' || (isset($escolar->abasket) ? $escolar->abasket : '') == '1')
                            checked
                        @endif> <label for="abasket"> Basketball</label></td>
                    <td><input type="checkbox" name="atennis" id="atennis" value="1"
                    @if(old('atennis') == '1' || (isset($escolar->atennis) ? $escolar->atennis : '') == '1')
                            checked
                        @endif> <label for="atennis"> Tennis</label></td>
                    <td><input type="checkbox" name="amanuales" id="amanuales" value="1"
                    @if(old('amanuales') == '1' || (isset($escolar->amanuales) ? $escolar->amanuales : '') == '1')
                            checked
                        @endif> <label for="amanuales"> Trabajos Manuales</label></td>
                    <td><input type="checkbox" name="aoratoria" id="aoratoria" value="1"
                    @if(old('aoratoria') == '1' || (isset($escolar->aoratoria) ? $escolar->aoratoria : '') == '1')
                            checked
                        @endif> <label for="aoratoria"> Oratoria</label></td>
                </tr>
                <tr>
                    <td><input type="checkbox" name="avolley" id="avolley" value="1"
                    @if(old('avolley') == '1' || (isset($escolar->avolley) ? $escolar->avolley : '') == '1')
                            checked
                        @endif> <label for="avolley"> Volleyball</label></td>
                    <td><input type="checkbox" name="aatletismo" id="aatletismo" value="1"
                    @if(old('aatletismo') == '1' || (isset($escolar->aatletismo) ? $escolar->aatletismo : '') == '1')
                            checked
                        @endif> <label for="aatletismo"> Atletismo</label></td>
                    <td><input type="checkbox" name="adomestico" id="adomestico" value="1"
                    @if(old('adomestico') == '1' || (isset($escolar->adomestico) ? $escolar->adomestico : '') == '1')
                            checked
                        @endif> <label for="adomestico"> Trabajo Domestico</label></td>
                    <td><input type="checkbox" name="aanimales" id="aanimales" value="1"
                    @if(old('aanimales') == '1' || (isset($escolar->aanimales) ? $escolar->aanimales : '') == '1')
                            checked
                        @endif> <label for="aanimales"> Cuidar Animales</label></td>
                    <td><input type="checkbox" name="adibujo" id="adibujo" value="1"
                    @if(old('adibujo') == '1' || (isset($escolar->adibujo) ? $escolar->adibujo : '') == '1')
                            checked
                        @endif><label for="adibujo"> Dibujo y Pintura</label></td>
                </tr>
                <tr>
                    <td><input type="checkbox" name="afiestas" id="afiestas" value="1"
                    @if(old('afiestas') == '1' || (isset($escolar->afiestas) ? $escolar->afiestas : '') == '1')
                            checked
                        @endif> <label for="afiestas"> Fiestas y Reuniones Sociales</label></td>
                    <td><input type="checkbox" name="acientificos" id="acientificos" value="1"
                    @if(old('acientificos') == '1' || (isset($escolar->acientificos) ? $escolar->acientificos : '') == '1')
                            checked
                        @endif> <label for="acientificos"> Estudios Cientificos</label></td>
                    <td><input type="checkbox" name="aenfermos" id="aenfermos" value="1"
                    @if(old('aenfermos') == '1' || (isset($escolar->aenfermos) ? $escolar->aenfermos : '') == '1')
                            checked
                        @endif> <label for="aenfermos">Cuidar Enfermos</label></td>
                    <td><input type="checkbox" name="aotros" id="aotros" value="1"
                    @if(old('aotros') == '1' || (isset($escolar->aotros) ? $escolar->aotros : '') == '1')
                            checked
